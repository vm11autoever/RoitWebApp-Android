...
// Foreground일 때 여기서 아래에서 noti를 임의로 발생시켜 줘야한다.
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        if (0 < remoteMessage.data.size) {
            link = remoteMessage.data["link"]//koup 링크 기준.
...
//        sendNotification(link, title, "상세내용") //아마 noti를 눌렀을떄 발생을 위해 처리한듯.
        sendNotification("www.nate.com", title, "onMessageReceived에서 처리되는 중.") //아마 noti를 눌렀을떄 발생을 위해 처리한듯.
    }
...
//        params.putExtra("link-case", link)
        params.putExtra("push", link)
//        params.putEx("push_url", link)
        var str = params.getStringExtra("push")
        Log.d("vm11",""+str)
